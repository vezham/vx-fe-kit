...
import { SubTaskDrawerProps, useSubTaskDrawerProps } from './types'
...
export const SubTaskDrawer = ({
  ...
  const handleFileUpload = (files: FileList | null) => {
    ...
    const uploaded: any[] = Array.from(files).map(file => ({
  ...
  const removeAttachment = (id: string) => {
    setForm(f => ({
      ...f,
      attachments: f.attachments.filter((a: any) => a.id !== id)
    }))
  }
        ...
        <DrawerBody {...getDrawerBodyProps()}>
          ...
          <>
            ...
            <Select
              ...
              onSelectionChange={keys =>
                setForm(f => ({ ...f, status: [...keys][0] as string }))
              }
              {...getSelectProps()}
              startContent={
                form.status && (
