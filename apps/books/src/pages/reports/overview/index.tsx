...
export default function Component() {
  return (
    <dl className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
      {data.map(
        (
          { title, value, change, changeType, iconName, trendChipPosition },
          index
        ) => (
          <Card key={index} className="border-default-200 border shadow-none">
            <div className="flex p-4">
              ...
              <div className="flex flex-col gap-y-2">
                <dt className="text-deafult-500 mx-4 text-xl leading-7 font-medium">
                  {title}
                </dt>
                <dd className="text-default-700 px-4 text-2xl font-medium">
                  {value}
                </dd>
              </div>
