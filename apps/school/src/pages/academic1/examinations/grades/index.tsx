...
export default function AllClassesPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('academic:grades:create', openAddSchedule)
    return () =>
      window.removeEventListener('academic:grades:create', openAddSchedule)
  }, [openDrawer])
  ...
  const saveClass = () => {
    ...
    const percentage = form.percentage.trim()
...
function ClassDrawer({
  ...
  const drawerTitle = mode === 'create' ? 'Add Grades' : row ? `#${row.id}` : ''
  ...
  return (
    <div className={classNames.form}>
      ...
      {formErrors.marksfrom && (
        <p className={classNames.selectError}>{formErrors.marksfrom}</p>
      )}
      ...
      {formErrors.marksupto && (
        <p className={classNames.selectError}>{formErrors.marksupto}</p>
      )}
      ...
      <Select
        ...
        onChange={value => onFormChange('points', value ? String(value) : '')}>
      ...
      {formErrors.points && (
        <p className={classNames.selectError}>{formErrors.points}</p>
      )}
