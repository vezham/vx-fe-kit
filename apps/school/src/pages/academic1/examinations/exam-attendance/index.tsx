...
export default function ExamAttendancePage() {
  ...
  const sortedRows = useMemo(() => {
    return [...filteredRows].sort((firstRow, secondRow) => {
      const first = firstRow[sortDescriptor.column as keyof AttendanceRow]
      const second = secondRow[sortDescriptor.column as keyof AttendanceRow]
      const comparison =
  ...
  const copyAttendanceLink = (row: AttendanceRow) => {
    const classUrl = getAttendanceUrl(row, mode === 'edit' ? 'edit' : 'view')
        ...
        <div className={classNames.headerRow}>
          ...
          <div className={classNames.toolbarActions}>
                ...
                <Surface className={classNames.datePopover}>
                  {isCustomDateRangeOpen ? (
                    <div
                      ...
                      <DateRangePicker
                        defaultOpen
                        aria-label="Attendance custom date range"
                        ...
                        <DateRangePicker.Popover>
                          <RangeCalendar aria-label="Attendance custom date range">
            ...
            <Dropdown>
              ...
              <Dropdown.Popover>
                <Dropdown.Menu aria-label="Sort attendance">
                  ...
                  {/* <Table.Cell>
                    <div
                      ...
                      <Button
                        ...
                        variant="outline"
                        onPress={() => deleteAttendance(row.id)}>
                        <Icon
...
function getStudentSecondaryText(row: AttendanceRow) {
  if (row.rollNo) {
                ...
                <div className={classNames.drawerActions}>
                  ...
                  {showNavigation && (
                    <>
                      ...
                      <Button
                        isIconOnly
                        aria-label="Previous attendance"
              ...
              {isFormMode ? (
                <div className={classNames.drawerFormFooterActions}>
                  ...
                  <Button onPress={onSave}>
                    {mode === 'create' ? 'Add Exam Attendance' : 'Save'}
                  </Button>
                </div>
              ) : (
...
function AttendanceDetails({ row }: AttendanceDetailsProps) {
  if (!row) {
    return null
  }
...
function getAttendanceTags(row: AttendanceRow) {
  return [
...
function validateAttendanceForm(form: AttendanceFormState) {
...
function createNextAttendanceId(rows: AttendanceRow[]) {
  const nextNumber =
