...
const getHomeworkBasePath = (pathname: string) => {
  const routeIndex = pathname.indexOf(homeworkRoutePath)

  if (routeIndex < 0) {
    return pathname.replace(/\/$/, '')
  }

  return pathname.slice(0, routeIndex + homeworkRoutePath.length)
}

const getRowIdFromPath = (pathname: string) => {
  const basePath = getHomeworkBasePath(pathname)
...
export function useHomeworkPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(
    () => new Set(homeworkColumnOptions.map(column => column.key))
  )
  ...
  const filteredRows = useMemo(() => {
    ...
    return data.filter(row => {
      const matchesQuery =
        ...
        row.homeworkdate.toLowerCase().includes(query) ||
        row.submissiondate.toLowerCase().includes(query) ||
        row.status.toLowerCase().includes(query) ||
        row.createdBy.name.toLowerCase().includes(query) ||
        (row.createdBy.secondaryText?.toLowerCase().includes(query) ?? false)
      const matchesDate =
        !activeDateRange ||
        isISODateInRange(
          row.createdAt,
      ...
      return (
        ...
        (!filters.homeworkdate || row.homeworkdate === filters.homeworkdate) &&
        (!filters.submissiondate ||
          row.submissiondate === filters.submissiondate) &&
  ...
  const sortedRows = useMemo(() => {
    return [...filteredRows].sort((firstRow, secondRow) => {
      const first = getSortableValue(firstRow, sortDescriptor.column)
      const second = getSortableValue(secondRow, sortDescriptor.column)
      const comparison =
  ...
  const activeSortLabelText = activeSortLabel
  ...
  const updateDrawerQuery = useCallback(
    (nextState: DrawerQueryState | null, replace = false) => {
      ...
      const basePath = getHomeworkBasePath(url.pathname)
  ...
  useEffect(() => {
    ...
    window.addEventListener('academic:homework:create', openAddSchedule)
    return () =>
      window.removeEventListener('academic:homework:create', openAddSchedule)
  }, [openDrawer])
  ...
  const updateVisibleColumns = useCallback((columns: Set<string>) => {
  ...
  const saveClass = () => {
    ...
    const homeworkdate = form.homeworkdate
    const submissiondate = form.submissiondate
    const attachments = form.attachments.trim()
  ...
  return {
    ...
    toolbar: {
      activeDateLabel,
      activeSortLabel: activeSortLabelText,
      datePreset,
      draftFilters,
