...
const classesRoutePath = '/academic/classes/allclasses'

const getClassesBasePath = (pathname: string) => {
  const routeIndex = pathname.indexOf(classesRoutePath)

  if (routeIndex < 0) {
    return pathname.replace(/\/$/, '')
  }

  return pathname.slice(0, routeIndex + classesRoutePath.length)
}

const getRowIdFromPath = (pathname: string) => {
  const basePath = getClassesBasePath(pathname)
...
const getSortLabel = (column: SortDescriptor['column']) => {
  return (
    ...
    allClassesColumnOptions.find(option => option.key === column)?.label ??
    'Sort'
  )
}
...
export function useClassesPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<
    Set<AllClassesColumnKey>
  >(() => new Set(allClassesColumnOptions.map(column => column.key)))
  ...
  const updateDrawerQuery = useCallback(
    (nextState: DrawerQueryState | null, replace = false) => {
      ...
      const basePath = getClassesBasePath(url.pathname)
  ...
  const updateVisibleColumns = useCallback(
    (columns: Set<AllClassesColumnKey>) => {
  ...
  const getClassUrl = (
    ...
    url.searchParams.delete('id')
    url.pathname = `${getClassesBasePath(url.pathname)}/${encodeURIComponent(row.id)}`
    url.hash = ''
