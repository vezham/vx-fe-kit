...
const getSubjectBasePath = (pathname: string) => {
  const routeIndex = pathname.indexOf(subjectRoutePath)

  if (routeIndex < 0) {
    return pathname.replace(/\/$/, '')
  }

  return pathname.slice(0, routeIndex + subjectRoutePath.length)
}

const getRowIdFromPath = (pathname: string) => {
  const basePath = getSubjectBasePath(pathname)
...
const getSortLabel = (column: SortDescriptor['column']) => {
  return (
    ...
    subjectColumnOptions.find(option => option.key === column)?.label ??
    'Sort'
  )
}
...
export function useSubjectPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(
    () => new Set(subjectColumnOptions.map(column => column.key))
  )
  ...
  const updateDrawerQuery = useCallback(
    (nextState: DrawerQueryState | null, replace = false) => {
      ...
      const basePath = getSubjectBasePath(url.pathname)
