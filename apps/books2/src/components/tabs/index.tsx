    ...
    <Component {...getBaseProps()}>
      <Surface
        ...
        <Tabs
          ...
          selectedKey={selectedKey}
          onSelectionChange={key => handleChange(String(key))}>
          <Tabs.ListContainer className={slots.listContainer({ class: classNames?.listContainer })}>
            <div className={slots.scrollContainer({ class: classNames?.scrollContainer })}>
              <Tabs.List
                aria-label="Options"
                className={slots.tabsList({ class: classNames?.tabsList })}>
                {tabs.map(tab => (
                  <Tabs.Tab key={tab.key} id={tab.key} className={slots.tab({ class: classNames?.tab })}>
                    {tab.title}
                    <Tabs.Indicator className={slots.indicator({ class: classNames?.indicator })} />
                  </Tabs.Tab>
                ))}
              </Tabs.List>
            </div>
